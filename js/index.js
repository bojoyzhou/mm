...
class Calculator {
    ...
    balance(node) {
        ...
        let firstChild = node.children[0],
            ...
            firstChildBcr = this.getElem(firstChild).getBoundingClientRect(),
            lastChildBcr = this.getElem(lastChild).getBoundingClientRect()
        let t_height,
            t_width,
            t_left,
            t_right,
        ...
        for (let i = 1; i < len; i++) {
            let last = node.children[i - 1],
                lastBcr = this.getElem(last).getBoundingClientRect()

            current = node.children[i]
            currentBcr = this.getElem(current).getBoundingClientRect()
        ...
        left = firstChildBcr.left - PW - node.width
        top = (firstChildBcr.top + currentBcr.top) / 2
        node.setPos(left, top)


        t_top = firstChild.t_top
        t_left = Math.min(firstChild.t_left, node.left)
        t_bottom = lastChildBcr.bottom
